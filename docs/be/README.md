 ...
 * Для `s3` патрабуецца ўласцівасць `bucket` . Дадатковыя ўласцівасці:
 ...
 ### Узровень часопіса
 Выкарыстоўвайце зменную асяроддзя `MOBILETTO_LOG_LEVEL` , каб усталяваць узровень журнала, выкарыстоўваючы адзін
 ...
 Узровень па змаўчанні - `error` . Самы шматслоўны ўзровень - `silly` , хоць у цяперашні час mobiletto
 не рэгіструе на ўзроўнях, ніжэйшых за `debug`
